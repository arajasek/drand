...
// cancellation of sync requests if not progessing, performs rate limiting of
// sync requests.
type SyncManager struct {
	...
	// updated with each new beaocn we receive from sync
...
func (s *SyncManager) Run() {
	...
	for {
		...
		case request := <-s.newReq:
			...
			if err != nil {
				s.log.Debug("sync_manager", "unable to fetch from store", "err", err)
				continue
			}
			// do we really need a sync request ?
			if request.upTo > 0 && last.Round >= request.upTo {
				s.log.Debug("sync_manager", "request already filled", "last", last.Round, "request", request.upTo)
				continue
			}
			...
			// We always give a delay of 2periods since the one next to "now"
			// might not be exactly ready yet so only after 2 periods we know we
			// must have gotten some data.
			upperBound := lastRoundTime + int(s.period.Seconds())*s.factor
			if upperBound < int(s.clock.Now().Unix()) {
				// we havent received a new block in a while
...
func (s *SyncManager) sync(ctx context.Context, request requestInfo) {
	s.log.Debug("sync_manager", "starting new sync", "up_to", request.upTo, "nodes", peersToString(request.nodes))
...
func (s *SyncManager) tryNode(global context.Context, upTo uint64, n net.Peer) bool {
	...
	if err != nil {
		s.log.Debug("sync_manager", "unable_to_sync", "with_peer", n.Address(), "err", err)
		return false
	}

	s.log.Debug("sync_manager", "start_follow", "with_peer", n.Address(), "from_round", last.Round+1)
	...
	for {
		...
		case beaconPacket := <-beaconCh:
			...
			if err := s.store.Put(beacon); err != nil {
				s.log.Debug("sync_manager", "unable to save", "with_peer", n.Address(), "err", err)
				return false
			}
			last = beacon
			if last.Round == upTo {
				s.log.Debug("sync_manager", "syncing finished to", "round", upTo)
				return true
			}
		case <-cnode.Done():
			// it can be the remote note that stopped the syncing or a network
			// error with it
			s.log.Debug("sync_manager", "follow canceled", "source", "remote", "err?", cnode.Err())
			// we still go on with the other peers
			return false
		case <-global.Done():
			// or a cancellation of the syncing process itself, maybe because
			// it's stuck
			s.log.Debug("sync_manager", "follow canceled", "source", "global", "err?", global.Err())
			// we stop
			return false
		}
	}
...
// SyncRequest is an interface representing any kind of request to sync. Those
// exists in the protocol API and the public API.
type SyncRequest interface {
	GetFromRound() uint64
}

// SyncStream is an interface represent any kind of stream to send beacons to.
// Those exists in the protocol API and the public API.
